...
export function intersectLineLine([[x1, y1], [x2, y2]]: Line, [[x3, y3], [x4, y4]]: Line): Vec2 | undefined {
  const k = ((x1 - x2) * (y3 - y4) - (y1 - y2) * (x3 - x4));
  const t = ((x1 - x3) * (y3 - y4) - (y1 - y3) * (x3 - x4)) / k;
  const u = - ((x1 - x2) * (y1 - y3) - (y1 - y2) * (x1 - x3)) / k;
  if (0 <= t && t <= 1 && u <= 0 && u <= 1) {
    return [
      x1 + t * (x2 - x1),
      y1 + t * (y2 - y1),
